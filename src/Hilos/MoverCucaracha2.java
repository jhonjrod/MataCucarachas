...
public class MoverCucaracha2 extends Thread {
    ...
    public void run(){
        while(true){
            //System.out.println("movercucaracha2 inicio");
            ...
            label.setBounds(-300, 110, 180, 95);
            //System.out.println(y);
            j.add(label);
            MoverCucaracha.cucarachas.add(label);
            for(int i=-300;i<=1700 && hilosigue;i+=16){try {
                //mueve la cucaracha hacia la derecha, poco a poco
                Thread.sleep(500);
                } catch (InterruptedException ex) {
                    Logger.getLogger(MoverCucaracha.class.getName()).log(Level.SEVERE, null, ex);
                }
                label.setBounds(i, 110, 180, 95);
                if(i==1700){
                    j.remove(label);
                    j.revalidate();
                    j.repaint();
                    System.out.println("Ultima ejecucion del for - fila cucaracha 2");
    ...
    public MoverCucaracha2(JFrame j) {
        //System.out.println("constructor movercucaracha2 inicio");
        this.j = j;
    }
